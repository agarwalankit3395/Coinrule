...
const oneYearPackage = ["Efficient Crypto Arbitrage Opportunities",
    "Real-time Price Monitoring",
    "User-Friendly Interface",
    "Customised able Alerts (upcoming)",
    "Arbitrage Strategy Insights",
...
const PricingCorrily = () => {
    ...
    const handleOneMonthPackage = async () => {
        ...
        if (checkAllowance.toString() < Monthprice) {
            const res = await info?.usdt_token.increaseAllowance(Smart_Contract_Address, yearPrice.toString());
            console.log(res);
        }
            ...
            <Container maxWidth={'lg'}>
                <Grid container spacing={2}>
                    <Grid item md={5} sm={12} xs={12} margin={'auto'}>
                        <Box padding={'1.5rem'} borderRadius={'12px'} sx={{ background: 'linear-gradient(90deg,#852ef6 0%,#00ffd4 100%)' }}>
                            <Box>
                                <Typography variant='h4' fontWeight={'500'} color="#fff">Monthly Plan - 25USDT</Typography>
                            </Box>
                            <Box paddingY={'1.5rem'}>
                                <List>
                                    {oneMonthPackage.map((data) => (
                                        <ListItem>
                                            ...
                                            <Typography variant='h6' fontWeight={'400'} color='#fff'>{data}</Typography>
                                        </ListItem>
                                    ))}
                            ...
                            <Box paddingTop={'2rem'}>
                                <Box sx={{ gap: '10px', display: 'flex', alignItems: 'center' }}>
                                    <Button onClick={handleOneMonthPackage} endIcon={<AddOutlinedIcon sx={{ color: '#fff' }} />} variant="contained" sx={{ boxShadow: 'none', fontSize: '20px', borderRadius: '50px', background: 'linear-gradient(180deg,rgb(255,229,64) 0%,rgb(255,185,57) 100%)', paddingX: '2rem', color: '#fff', textTransform: 'capitalize' }}>
                                        {allowancePrice > 0 ? "Subscribe" : "Allowance" }
                                    </Button>
                                    <Button onClick={handleUnsubscribe} endIcon={<AddOutlinedIcon sx={{ color: '#fff' }} />} variant="contained" sx={{ boxShadow: 'none', fontSize: '20px', borderRadius: '50px', paddingX: '2rem', color: '#fff', textTransform: 'capitalize' }}>
                                        unsubscribe
                                    </Button>
                                </Box>
                            </Box>
                    ...
                    <Grid item md={5} sm={12} xs={12} margin={'auto'}>
                        <Box padding={'1.5rem'} borderRadius={'12px'} sx={{ background: 'linear-gradient(90deg,#606060 0%,#585858 100%)' }}>
                            <Box>
                                <Typography variant='h4' fontWeight={'500'} color="#fff">Year Plan - 200 USDT</Typography>
                            </Box>
                            <Box paddingY={'1.5rem'}>
                                <List>
                                    {oneYearPackage.map((yeardata) => (
                                        <ListItem>
                                            ...
                                            <Typography variant='h6' fontWeight={'400'} color='#fff'>{yeardata}</Typography>
                                        </ListItem>
                                    ))}
                                </List>
                            </Box>
                            <Box paddingTop={'2rem'}>
                                <Box sx={{ gap: '10px', display: 'flex', alignItems: 'center' }}>
                                    <Button onClick={handleOneYearPackage} endIcon={<AddOutlinedIcon sx={{ color: '#fff' }} />} variant="contained" sx={{ boxShadow: 'none', fontSize: '20px', borderRadius: '50px', background: 'linear-gradient(180deg,rgb(255,229,64) 0%,rgb(255,185,57) 100%)', paddingX: '2rem', color: '#fff', textTransform: 'capitalize' }}>
                                        {allowancePrice > 0 ? "Subscribe" : "Allowance" }
                                    </Button>
                                    <Button onClick={handleUnsubscribe} endIcon={<AddOutlinedIcon sx={{ color: '#fff' }} />} variant="contained" sx={{ boxShadow: 'none', fontSize: '20px', borderRadius: '50px', paddingX: '2rem', color: '#fff', textTransform: 'capitalize' }}>
                                        unsubscribe
